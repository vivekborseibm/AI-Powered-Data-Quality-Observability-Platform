...
Checks performed:
  - check_nulls              : null % per column
  - check_duplicates         : duplicate row % across key columns
  - check_schema             : column presence and dtype conformity
  - check_ranges             : numeric outliers via z-score
  - check_freshness          : age of the most recent record
  - check_referential_integrity : FK presence in reference table
...
class DataQualityChecker:
    ...
    Usage::
        ...
        checker = DataQualityChecker(spark, metrics_table="dq_project.quality_metrics")
        metrics = checker.run_all_checks(df, table_name="dq_project.silver_orders", layer="silver")
        checker.write_metrics(metrics)
    """
    ...
    def run_all_checks(
        ...
        timestamp_column: str | None = "order_date",
        ref_checks: list[dict[str, Any]] | None = None,
    ) -> list[MetricRecord]:
        results: list[MetricRecord] = []
        results += self.check_nulls(df, table_name, layer)
        results += self.check_duplicates(df, table_name, layer, key_columns=key_columns)
        if expected_schema:
            results += self.check_schema(df, table_name, layer, expected_schema)
        results += self.check_ranges(df, table_name, layer)
        if timestamp_column:
            results += self.check_freshness(df, table_name, layer, timestamp_column)
        if ref_checks:
            for rc in ref_checks:
                results += self.check_referential_integrity(df, table_name, layer, **rc)
        return results
    ...
    def check_nulls(self, df: DataFrame, table_name: str, layer: str) -> list[MetricRecord]:
        ...
        for col_name in df.columns:
            ...
            records.append(MetricRecord(
                ...
                check_name="null_rate", column_name=col_name,
    ...
    def check_schema(
        ...
        for expected_field in expected_schema.fields:
            ...
            records.append(MetricRecord(
                table_name=table_name, layer=layer,
                check_name="schema_conformity", column_name=expected_field.name,
    ...
    def check_ranges(self, df: DataFrame, table_name: str, layer: str) -> list[MetricRecord]:
        ...
        for col_name in numeric_cols:
            ...
            if std_val and std_val > 0:
                ...
                outlier_rate = outlier_count / df.count()
            else:
                outlier_rate = 0.0
            records.append(MetricRecord(
                table_name=table_name, layer=layer,
                check_name="outlier_rate", column_name=col_name,
                metric_value=round(outlier_rate, 4),
                threshold=0.01,
                passed=outlier_rate <= 0.01,
                details=f"mean={mean_val:.2f}, std={std_val:.2f}" if std_val else "std=0",
            ))
        return records
    ...
    def check_freshness(
        ...
        if latest is None:
            ...
            age_hours = (datetime.datetime.utcnow() - latest).total_seconds() / 3600
        return [MetricRecord(
            table_name=table_name, layer=layer,
            check_name="freshness_hours", column_name=timestamp_column,
            metric_value=round(age_hours, 2),
            threshold=FRESHNESS_MAX_HOURS,
            passed=age_hours <= FRESHNESS_MAX_HOURS,
            details=f"latest record at {latest}",
        )]

    def check_referential_integrity(
        self, df: DataFrame, table_name: str, layer: str,
        fk_column: str, ref_table: str, ref_column: str,
    ) -> list[MetricRecord]:
        ref_df = self.spark.read.format("delta").table(ref_table)
        total = df.count()
        orphans = df.join(ref_df, df[fk_column] == ref_df[ref_column], "left_anti").count()
        orphan_rate = orphans / total if total else 0.0
        return [MetricRecord(
            table_name=table_name, layer=layer,
            check_name="referential_integrity", column_name=fk_column,
            metric_value=round(orphan_rate, 4),
            threshold=0.0,
            passed=orphan_rate == 0.0,
            details=f"{orphans} rows with no match in {ref_table}.{ref_column}",
        )]
